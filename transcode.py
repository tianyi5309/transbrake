...
def encode(inmov, outdir, outname):
    ...
    # Transcode
    start = time.time()
    subprocess.check_output(['ffmpeg', '-i', inmov] + chosen_streams + ['-vcodec', 'libx264', '-pix_fmt', 'yuv420p', '-deinterlace', '-x264opts', 'analyse=none:ref=1:rc-lookahead=30:no-scenecut', '-crf', '18', '-maxrate', '8M', '-bufsize', '8M', '-preset', 'fast', '-tune', 'film', '-filter:v', 'hqdn3d=1:1:4:4', '-profile:v', 'high', '-level', '4.2', '-g', '60', '-acodec', 'aac', '-ac', '2', '-b:a', '192k', '-af', 'aresample=async=1000', '-scodec', 'mov_text', '-movflags', '+faststart', outtmp])
