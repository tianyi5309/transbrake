...
def encode(inmov, outdir, outname):
    ...
    # Transcode
    start = time.time()
    subprocess.check_output(['ffmpeg', '-i', inmov] + chosen_streams + ['-vcodec', 'libx264', '-x264opts', 'analyse=none:ref=1:rc-lookahead=30', '-crf', '18', '-maxrate', '6M', '-bufsize', '6M', '-preset', 'fast', '-tune', 'film', '-filter:v', 'hqdn3d=1:1:4:4', '-profile:v', 'high', '-level', '4.2', '-pix_fmt', 'yuv420p','-acodec', 'aac', '-b:a', '256k', '-scodec', 'mov_text', '-movflags', '+faststart', outtmp])
