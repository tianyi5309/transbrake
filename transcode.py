...
def encode(inmov, outmov):
    ...
    subprocess.check_output(['ffmpeg', '-i', inmov, '-map', '0:v', '-map', '0:a', '-map', '0:s', '-map', '-0:m:language:rus?', '-map', '-0:m:language:ukr?', '-map', '-0:m:codec_name:hdmv_pgs_subtitle', '-vcodec', 'libx264', '-crf', '18', '-maxrate', '8M', '-bufsize', '8M', '-preset', 'fast', '-acodec', 'aac', '-b:a', '256k', '-map_metadata', '-1', '-scodec', 'copy', '-movflags', 'faststart', outmov])
